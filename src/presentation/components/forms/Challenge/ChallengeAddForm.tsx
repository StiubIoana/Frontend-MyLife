...
import { useChallengeAddFormController } from "./ChallengeAddForm.controller";
import { isEmpty, isUndefined } from "lodash";

/**
 * Here we declare the Challenge add form component.
 * This form may be used in modals so the onSubmit callback could close the modal on completion.
 */
export const ChallengeAddForm = (props: { onSubmit?: () => void }) => {
    const { formatMessage } = useIntl();
    const { state, actions, computed } = useChallengeAddFormController(props.onSubmit); // Use the controller.
        ...
        <Stack spacing={4} style={{ width: "100%" }}>
            <Grid container item direction="row" xs={12} columnSpacing={4}>
                ...
                <Grid container item direction="column" xs={6} md={6}>
                    <FormControl
                        fullWidth
                        error={!isUndefined(state.errors.challengeAttributeNames)}
                    >
                        <FormLabel required>
                            <FormattedMessage id="globals.challengeAttribute" />
                        </FormLabel>
                        <OutlinedInput
                            type="challengeAttributeNames"
                            {...actions.register("challengeAttributeNames")}
                            placeholder={formatMessage(
                                { id: "globals.placeholders.textInput" },
                                {
                                    fieldName: formatMessage({
                                        id: "globals.challengeAttribute",
                                    }),
                                })}
                            autoComplete="none"
                        />
                        <FormHelperText
                            hidden={isUndefined(state.errors.challengeAttributeNames)}
                        >
                            {state.errors.challengeAttributeNames?.message}
                        </FormHelperText>
                    </FormControl>
                </Grid>
            </Grid>

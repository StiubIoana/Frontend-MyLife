...
const useInitRegisterForm = () => {
  ...
  const schema = yup.object().shape({
    // Use yup to build the validation schema of the form.
    name: yup
      .string()
      .required(
        formatMessage(
    ...
    email: yup
      ...
      .required(
        formatMessage(
          // Use formatMessage to get the translated error message.
          { id: "globals.validations.requiredField" },
          {
            fieldName: formatMessage({
              // Format the message with other translated strings.
              id: "globals.email",
            }),
          }
        )
      ) // The field is required and needs a error message when it is empty.
      .email(formatMessage({ id: "notifications.errors.emailInvalid" })) // This requires the field to have a email format.
    ...
    password: yup
      ...
      .min(
        5,
        formatMessage({ id: "notifications.errors.passwordMustBeAtLeast" })
      )
      ...
      .when("password", (password, field) =>
        password
          ? field
              ...
              .oneOf(
                [yup.ref("password")],
                formatMessage({ id: "notifications.errors.passwordsMustMatch" })
              )
          : field
      )
...
export const useRegisterFormController = (): RegisterFormController => {
  ...
  const {
    RegisterMutation: { mutation, key: mutationKey },
  } = useRegisterApi();
  const { mutateAsync: registerMutation, status } = useMutation(
    [mutationKey],
    mutation
  );
  ...
  const submit = useCallback(
    (
      data: RegisterFormModel // Create a submit callback to send the form data to the backend.
    ) =>
      registerMutation(data).then((result) => {
        toast(formatMessage({ id: "notifications.messages.registerSuccess" }));
        redirectToHome();
      }),
    [registerMutation, queryClient, redirectToHome, dispatch]
  );
